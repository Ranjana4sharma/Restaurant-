...
function categoryDisplayName(category: CategoryDTO, categories: CategoryDTO[]) {
...
export function MenuView() {
  ...
  const [latestAdditions, setLatestAdditions] = useState<ProductDTO[]>([]);
  const [mostDemanded, setMostDemanded] = useState<ProductDTO[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [homeData, s] = await Promise.all([fetchHome(), fetchSettings()]);
      setProducts(homeData.products);
      setCategories(homeData.categories);
      setLatestAdditions(homeData.latestAdditions);
      setMostDemanded(homeData.mostDemanded);
      setApprovedReviews(homeData.approvedReviews);
  ...
  const orderedSections = useMemo(() => {
    ...
    for (const c of sorted) {
      ...
      sections.push({
        ...
        chipLabel: categoryDisplayName(c, categories),
        title: `${categoryDisplayName(c, categories).toUpperCase()} (${items.length})`,
        items,
      });
    }
  ...
  const filterItems = useMemo(() => {
    ...
    return sortCategories(categories).map((c) => {
      ...
      return {
        ...
        name: categoryDisplayName(c, categories),
        count,
        image: c.image,
      };
    });
            ...
            <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
              {latestAdditions.map((p) => (
                <div key={`latest-${p._id}`} className="w-[18.5rem] shrink-0 sm:w-[20rem]">
                  <ProductCard product={p} onAdd={() => setModalProduct(p)} />
                </div>
              ))}
            </div>
          </section>
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  setReviewComment("");
                  setReviewRating("5");
                  setReviewMsg("Thanks! Review submitted for admin approval.");
                } catch {
