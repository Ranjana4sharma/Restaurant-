...
function loadInitial(): CartLine[] {
  ...
  try {
    ...
    // Migration: Filter out legacy IDs like 'p-1'
    return parsed
      .filter((p) => !p.productId.startsWith("p-"))
      .map((p) => ({
        ...p,
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const setOffer = useCallback((offer: any) => {
  ...
  const value = useMemo(
    ...
    [lines, itemCount, subtotal, appliedOffer, addLine, setQty, inc, dec, remove, clear, setOffer, removeOffer]
  );
