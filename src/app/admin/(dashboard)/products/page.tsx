...
export default function AdminProductsPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        isSignatureDish: form.isSignatureDish,
        isFamousDish: form.isFamousDish,
  ...
  const startEdit = (p: ProductDTO) => {
    ...
    setForm({
      ...
      isSignatureDish: Boolean(p.isSignatureDish),
      isFamousDish: Boolean(p.isFamousDish),
      variants:
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-sm text-[#f3e8c7]/70">
          ...
          <code className="rounded bg-[#d5b16a]/10 px-1">public/uploads</code> on the server.
        </p>
      </div>
      ...
      <div className="grid gap-6 sm:gap-8 lg:grid-cols-2">
        <form
          onSubmit={onSubmit}
          className="space-y-4 rounded-2xl border border-[#d5b16a]/20 bg-[#111111] p-4 sm:p-6 shadow-sm"
        >
          ...
          <div
            ...
            className={`cursor-pointer rounded-xl border-2 border-dashed border-[#d5b16a]/35 bg-gradient-to-br from-[#111111] to-[#0a0a0a] p-3 text-center transition hover:border-[#d5b16a]/60 hover:shadow-sm ${uploading ? "pointer-events-none opacity-60" : ""}`}
          >
            <Upload className="mx-auto h-8 w-8 text-[#e60000]" aria-hidden />
            <p className="mt-2 text-sm font-semibold text-[#f5d79e]">Upload product image</p>
          ...
          <div className="grid gap-2 sm:grid-cols-2 sm:gap-3">
            <label className="block text-xs font-semibold uppercase text-[#d5b16a]/70">
              ...
              <input
                required
                className="mt-1 w-full rounded-xl border px-3 py-2.5 sm:py-3 text-sm outline-none ring-[#d5b16a]/30 focus:ring-2"
                value={form.name}
            ...
            <label className="block text-xs font-semibold uppercase text-[#d5b16a]/70">
              Base price (₹){" "}
              <span className="normal-case font-normal text-[#d5b16a]/40">
          ...
          <label className="block text-xs font-semibold uppercase text-[#d5b16a]/70">
            <div className="flex items-center justify-between">
              <span>Category</span>
              <Link href="/admin/categories" className="text-[10px] font-bold text-[#e60000] hover:underline lowercase">
                Manage Categories →
              </Link>
            </div>
            <select
              required
              className="mt-1 w-full rounded-xl border border-[#d5b16a]/20 bg-[#0a0a0a] px-3 py-2 text-sm text-[#f3e8c7] focus:border-[#d5b16a] outline-none focus:border-[#d5b16a] focus:ring-2 focus:ring-[#d5b16a]/20 outline-none transition-all"
            ...
            {categories.length === 0 && !loading && (
              <p className="mt-1 text-[10px] normal-case font-medium text-amber-600">
                You haven't added any categories yet. Products need a category to be displayed.
              </p>
            )}
          </label>
          ...
          <label className="block text-xs font-semibold uppercase text-[#d5b16a]/70">
            Description
            <textarea
              className="mt-1 min-h-22 w-full rounded-xl border border-[#d5b16a]/20 bg-[#0a0a0a] px-3 py-2 text-sm text-[#f3e8c7] focus:border-[#d5b16a] outline-none"
          ...
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase text-[#d5b16a]/70">
              Product Image (Paste URL or Upload)
              <div className="flex mt-1 gap-2">
                <input
                  className="flex-1 rounded-xl border px-3 py-2 text-sm outline-none focus:border-[#d5b16a] focus:ring-1 focus:ring-[#d5b16a]/50"
                  value={form.image}
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
              ...
              {[
                ...
                <button
                  key={i}
                  type="button"
                  onClick={() => setForm({ ...form, image: url.replace('w=150', 'w=800') })}
          ...
          <div className="rounded-2xl border border-[#d5b16a]/20 bg-[#0a0a0a]/80 p-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-[#f3e8c7]/70">
                  Types / sizes
                </p>
                <p className="mt-0.5 text-xs text-[#d5b16a]/70">
              ...
              <button
                ...
                className="inline-flex shrink-0 items-center gap-1 rounded-full border border-[#d5b16a]/40 bg-[#111111] px-3 py-1.5 text-xs font-bold text-[#e60000] hover:bg-red-50"
              >
            ...
            {form.variants.length === 0 ? (
              <p className="mt-3 text-xs italic text-[#d5b16a]/70">
              ...
              <ul className="mt-3 space-y-2">
                {form.variants.map((row, i) => (
                  <li
                    key={i}
                    className="flex flex-wrap items-end gap-2 rounded-xl bg-[#111111] p-2 ring-1 ring-neutral-200"
                  >
                    <label className="min-w-0 flex-1 text-[10px] font-semibold uppercase text-[#d5b16a]/70">
                      Label
                      <input
                    ...
                    <label className="w-28 text-[10px] font-semibold uppercase text-[#d5b16a]/70">
                      ₹
                      <input
                        type="number"
                        min={0}
                    ...
                    <button
                      ...
                      className="mb-0.5 rounded-lg p-2 text-rose-500 hover:bg-red-50"
          ...
          <div className="grid gap-2 sm:grid-cols-5">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, isChefSpecial: e.target.checked })
                }
              />
              Chef Special
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, isSignatureDish: e.target.checked })
                }
              />
              Signature Dish
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={form.isFamousDish}
                onChange={(e) =>
                  setForm({ ...form, isFamousDish: e.target.checked })
                }
              />
              Most Famous
            </label>
          ...
          <div className="flex gap-2">
            ...
            {editingId && (
              <button
                type="button"
                onClick={reset}
                className="rounded-full border border-[#d5b16a]/30 px-6 py-2.5 text-sm font-semibold"
              >
                Cancel
              </button>
            )}
        ...
        <div className="rounded-2xl border border-[#d5b16a]/20 bg-[#fdf6e8]/40 p-4">
          <h3 className="mb-3 text-sm font-semibold text-[#f3e8c7]/70">
            Live card preview
          </h3>
          <div className="rounded-2xl bg-[#fdf6e8] p-3 shadow-md ring-1 ring-black/5">
            <div className="flex gap-3">
              <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-2xl bg-[#111111]">
                <Image
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="line-clamp-2 text-xs text-[#f3e8c7]/70">
      ...
      <div className="space-y-4 rounded-2xl border border-[#d5b16a]/20 bg-[#111111] p-4 shadow-sm">
        {/* Search and Filter Bar */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 sm:max-w-xs">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#d5b16a]/40" />
            <input
              type="text"
              placeholder="Search products…"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full rounded-lg border border-[#d5b16a]/20 bg-[#111111] pl-10 pr-3 py-2 text-sm focus:border-[#d5b16a] focus:outline-none focus:ring-2 focus:ring-[#d5b16a]/20"
            />
          </div>
          
          <div className="flex items-center gap-2">
            <label className="text-sm font-semibold text-[#f3e8c7]/70">Category:</label>
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="rounded-lg border border-[#d5b16a]/20 bg-[#111111] px-3 py-2 text-sm focus:border-[#d5b16a] focus:outline-none focus:ring-2 focus:ring-[#d5b16a]/20"
            >
              <option value="">{categories.length === 0 ? "No categories" : "All Categories"}</option>
              {categories.map((cat) => (
          ...
          {(searchTerm || selectedCategory) && (
            <button
              ...
              className="flex items-center gap-1 text-xs font-semibold text-[#f3e8c7]/70 hover:text-[#f5d79e]"
            >
              <X className="h-4 w-4" />
              Clear Filters
            </button>
        ...
        <p className="text-xs text-[#d5b16a]/70">
          Showing <span className="font-semibold text-[#f3e8c7]">{filteredProducts.length}</span> of{" "}
          <span className="font-semibold text-[#f3e8c7]">{products.length}</span> products
        </p>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-[#d5b16a]/20 bg-[#111111] shadow-sm">
        <table className="w-full min-w-160 text-left text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-[#d5b16a]/70">
                  Loading…
                </td>
              </tr>
              ...
              filteredProducts.map((p) => (
                <tr key={p._id} className="border-t border-[#d5b16a]/10">
